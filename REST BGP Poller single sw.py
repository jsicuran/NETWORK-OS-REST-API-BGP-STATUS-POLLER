...
def splitline():
    print(Fore.YELLOW + Style.BRIGHT + "-" * 165)
...
#######################################################################################
#########################REST BGP PEER STATUS CALL##################################################################
    bgp = session.get(
        f"https://{sw1}/rest/v10.11/system/vrfs/default/bgp_routers/65001/bgp_neighbors?attributes=status&depth=2&selector=status",
        verify=False)
    sw_resp = (bgp.json())
#########################################################################################################
    
####################################MAIN STATUS CHECK DECISION TREE################################################   
    print(Back.BLUE + Fore.WHITE + Style.BRIGHT +  '     DC-SITE NAME \u25ba SWITCH DC-L3-SW1 \u25c4 PEER CHECK    ')
    bottomline()
